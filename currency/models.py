...
class TypePair(models.Model):
    ...
    def in_orders_sum_buy(self):
        return self.warrant_orders_related.filter(buy__gte=1).exclude(Q(cancel=True) | Q(completed=True)).aggregate(Sum('amount')).values()[0] or _Zero
    def in_orders_sum_sell(self):
        return self.warrant_orders_related.filter(sale__gte=1).exclude(Q(cancel=True) | Q(completed=True)).extra(select={'total_sum':"sum(rate * amount)"},).get().total_sum or _Zero
    @property
    def sumamount(self):
